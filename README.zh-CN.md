...
4. **关闭**会先停 GPU，再做最终输出采集，之后才处理卷。默认关闭会把卷保留为**温卷**（见下文）；「关闭并删卷」则立即删除。如果采集失败、或一个交互过的会话没采到任何输出，卷会被保留以便恢复——丢弃它需要显式的 `discard_outputs` 操作。

### 温卷（Warm volumes）

启动过程中最昂贵的不是钱，而是等待——下载几十 GB 模型的那十几分钟。所以控制器默认在关闭后**保留已灌注模型的网络卷**而不是删除：

- **下次启动飞快。** 新会话需要相同模型时自动复用温卷：整个多机房预下载阶段被跳过，约 3–6 分钟就绪（冷启动要 7–12 分钟）。向导的工作流下拉和试算结果会用 ♨ 标记可复用的工作流。
- **花费以分计，且有界。** 存储费 $0.07/GB/月（40GB 约 $0.09/天）。闲置温卷在 `WARM_VOLUME_IDLE_TTL_HOURS`（默认 72 小时）后自动删除；每次复用都会续期。严格算账的话重新下载反而更便宜（CPU 费约 $0.03）——温卷买回的是你的等待时间，不是钱。
- **绝不藏匿。** 温卷在 Dashboard 上列出机房、大小、过期时间和累计费用，每个都带「立即删除」按钮。强制回收（花费上限、空闲超时）默认保留温卷——被打断的会话通常很快会重开。
- **过期内容是安全的。** 如果温卷内容已不匹配（换了模型、文件缺失），环境配置阶段会发现并自动切换到新灌注的候选机房。
