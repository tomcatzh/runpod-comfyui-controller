...
ZH: dict[str, str] = {
    ...
    "Shutdown (keep warm volume)": "关闭（保留温卷）",
    "Shutdown + delete volume": "关闭并删卷",
    "This stops GPU compute first, collects ComfyUI outputs through S3, then keeps the hydrated volume warm for fast relaunch (auto-deleted after the idle TTL).": "先停止 GPU 计费，经 S3 采集 ComfyUI 输出，然后保留已灌注模型的网络卷以便快速重启（闲置超时后自动删除）。",
    "Warm volumes": "温卷",
